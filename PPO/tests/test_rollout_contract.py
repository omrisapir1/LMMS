...
def test_default_rollout_contract_values() -> None:
    ...
    assert cfg.rollout.vllm_sync_every == 2
...
def test_action_scope_validation() -> None:
    ...
    assert validate_action_scope("ppo_only_z_tokens_and_verify") == "ppo_only_z_tokens_and_verify"
...
def test_scope_controls_digit_ppo_actions() -> None:
    ...
    assert is_ppo_action("ppo_only_z_tokens", "verify") is False
    assert is_ppo_action("ppo_full", "z") is True
    assert is_ppo_action("ppo_full", "digits") is True
    assert is_ppo_action("ppo_full", "verify") is False
    assert is_ppo_action("ppo_only_z_tokens_and_verify", "z") is True
    assert is_ppo_action("ppo_only_z_tokens_and_verify", "digits") is False
    assert is_ppo_action("ppo_only_z_tokens_and_verify", "verify") is True
