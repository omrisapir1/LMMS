...
def compute_rsft_losses(
    *,
    ...
    z_answer_allowed: List[int] = []
    seen = set()
    for tid in list(z_token_ids) + [int(answer_token_id)]:
        key = int(tid)
        if key not in seen:
            z_answer_allowed.append(key)
            seen.add(key)

    digits_allowed = [int(x) for x in digit_token_ids]
    verify_allowed = [int(x) for x in verify_token_ids]

    # Z and <ANSWER> share the same restricted vocab, but use disjoint masks.
    l_z = _restricted_masked_ce(
        logits=logits,
        labels=labels,
        mask=z_mask,
        allowed_token_ids=z_answer_allowed,
    )
