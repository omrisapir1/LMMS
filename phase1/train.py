...
def _print_stage_enter_example(
    *,
    ...
    print(raw.question)
    print("thoughts list (lines):")
    for i, t in enumerate(raw.thoughts[: dataset.max_thoughts]):
        print(f"[{i}] {t}")
    ...
    print("supervision targets:")
    print(f"target 5-digit string: {digit_str}")
    print(f"label token IDs for 5 digits: {label_digit_ids}")
    print(f"loss mask values for the 5 positions (after zero downsampling mask): {effective_loss_mask_values}")
    print("keep/drop per position:")
    for note in keep_drop_notes:
        print(f"- {note}")
