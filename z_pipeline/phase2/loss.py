...
class ZUsageKLLoss:
    """
    Prevents collapse of Z tokens by encouraging batch-level
    usage to be close to uniform.

    Computes:
        KL( q(z) || Uniform )

    where q(z) is the average Z usage over the batch.
    """
    ...
    def compute(
        ...
        z_mask: torch.Tensor,   # [B, K] (bool or 0/1)
    ) -> torch.Tensor:
        """
        Returns scalar loss.

        z_probs must be softmax probabilities (NOT logits).
        """
        if z_probs.ndim != 3:
            raise ValueError("z_probs must be [B, K, V]")
        if z_mask.ndim != 2:
            raise ValueError("z_mask must be [B, K]")

        B, K, V = z_probs.shape
        if V != self.vocab_size:
            raise ValueError(f"Expected V={self.vocab_size}, got {V}")

        # Convert mask to float and expand
        mask = z_mask.unsqueeze(-1).float()  # [B, K, 1]

        # Masked probabilities
        masked_probs = z_probs * mask  # [B, K, V]

        # Sum over batch and steps
        total_mass = mask.sum() + self.eps  # scalar
        q = masked_probs.sum(dim=(0, 1)) / total_mass  # [V]
