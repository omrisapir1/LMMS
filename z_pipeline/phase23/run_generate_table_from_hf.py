...
def _extract_five_digits(final_answer: Any) -> List[int]:
    ...
    if len(digits) == 0:
        raise ValueError(f"Could not parse any digits from final_answer: {final_answer!r}")
    if len(digits) > 5:
        raise ValueError(f"Could not parse <=5 digits from final_answer: {final_answer!r}")
    # final_answer may omit leading zeros (e.g. "8" -> 00008)
...
def main() -> None:
    ...
    p.add_argument("--checkpoint_dir", default="/workspace/LMMS/runs/phase23_gs/step_1000", help="Phase23 checkpoint dir (contains phase23_state.pt).")
